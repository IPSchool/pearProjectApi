...
//        $list = $eventMember->_list($where);
        if ($list['list']) {
            foreach ($list['list'] as &$item) {
                $item['memberList'] = [];
                $item['projectName'] = '';
                $members = EventsMember::where(['events_code' => $item['code']])->order('is_owner desc, status desc, id asc')->select();
                if ($members) {
    ...
    public function confirmList()
    {
        ...
        if ($list['list']) {
            foreach ($list['list'] as &$item) {
                $item['memberList'] = [];
                $item['projectName'] = '';
                $members = EventsMember::where(['events_code' => $item['code']])->select();
                if ($members) {
                    $item['memberList'] = $members;
                }
                $project = \app\common\Model\Project::where('code', $item['project_code'])->field('name')->find();
    ...
    {
        $data = request_only('project_code,title,description,begin_time,end_time,all_day,position');
