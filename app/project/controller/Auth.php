...
class Auth extends BasicApi
{
    ...
    protected function _apply_getnode($auth)
    {
        ...
        $checkedList = [];
        $all = $this->_apply_filter(ToolsService::arr2tree($nodes, 'node', 'pnode', 'children'), $checkedList);
    ...
    {
        //todo 优化
        list($data, $post) = [[], request_only('action,id,nodes')];
     ...
     * @return array
     */
    protected function _apply_filter($nodes, &$checkedList, $level = 1)
    {
        foreach ($nodes as $key => $node) {
            if (!empty($node['children']) && is_array($node['children'])) {
                $node[$key]['children'] = $this->_apply_filter($node['children'], $checkedList, $level + 1);
            } else {
                if ($node['checked']) {
    ...
    public function add()
    {
        ...
        $currentOrganizationCode = getCurrentOrganizationCode();
        $params = request_only('title,desc,id');
    ...
    public function edit()
    {
        ...
        $result = $this->model->_edit(request_only('title,desc,id'));
        if ($result) {
            $this->success('');
        }
    ...
    {
        $result = $this->model->_edit(request_only('status,id'));
        if ($result) {
            $this->success('');
        }
        $this->error("权限禁用失败，请稍候再试！");
    ...
    {
        $currentOrganizationCode = getCurrentOrganizationCode();
        $params = request_only('is_default,id');
    ...
    public function del()
    {
        $result = $this->model->del(request_only('id'));
        if ($result) {
            $this->success("权限删除成功！", '');
        }
