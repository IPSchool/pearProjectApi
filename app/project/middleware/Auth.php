...
class Auth
{
    ...
    {
        $parts = explode('/', trim($request->pathinfo(), '/'));
        // pathinfo: project/login/index — 路由中间件执行时 controller() 可能尚未赋值
        if (($parts[0] ?? '') === 'project' && ($parts[1] ?? '') === 'login') {
            return $next($request);
        }
        ...
        $action = $request->action(true) ?: ($parts[2] ?? '');
        $node = NodeService::parseNodeStr("{$module}/{$controller}/{$action}");

        $access = $this->buildAuth($node);
